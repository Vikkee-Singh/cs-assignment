...
exports.login = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let user = yield db.query(`SELECT * from users WHERE email="${req.body.email}"`);
        if (user.length === 0)
        ...
        const isTokenExists = yield db.query(`SELECT * from tokens WHERE email="${req.body.email}"`);
        if (isTokenExists.length === 0) {
            yield db.query(`INSERT INTO tokens (userId, name, email, token) VALUES (${user[0].id},"${user[0].name}","${user[0].email}","${token}")`);
        }
        else {
            yield db.query(`UPDATE tokens SET token="${token}" WHERE userId=${user[0].id}`);
        }
...
exports.signup = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = new User_1.default(req.body.name, req.body.email, req.body.password);
        const validationError = user.validateUser();
        if (validationError.length) {
            throw JSON.stringify(validationError);
        }
        let db = yield database_1.default.Get();
        const isUserExist = yield db.query(`SELECT * from users WHERE email="${req.body.email}"`);
        if (isUserExist.length !== 0) {
            throw JSON.stringify("User Allready Exist");
        }
        const encrptPassword = bcrypt.hashSync(req.body.password, 10);
        yield db.query(`INSERT INTO users (name, email, password) VALUES ("${req.body.name}", "${req.body.email}","${encrptPassword}")`);
        res.status(200).send({
            done: true,
            statusCode: 200,
